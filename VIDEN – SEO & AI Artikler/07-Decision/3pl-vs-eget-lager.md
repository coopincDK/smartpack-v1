---
primary_keyword: 3PL vs eget lager
secondary_keywords: tredjepartslogistik, outsource lager, 3PL fordele ulemper, lager outsourcing
cluster: 07-Decision
article_type: beslutning
---

# 3PL vs. eget lager: Hvad passer til din webshop?

Du rammer 150 ordrer/dag. Lagerplads er ved at løbe tør. En 3PL-leverandør lover "fuld skalering" for 85 kr./ordre. Det lyder godt – indtil du regner: 150 ordrer × 85 kr. × 250 dage = **3.187.500 kr./år**. Eget lager med WMS: 850.000 kr./år. Forskel: **2,3 mio. kr.**

Men det modsatte kan også ske: Du bygger eget lager ved 80 ordrer/dag. Faste omkostninger: 45.000 kr./md. Volumen falder til 50 ordrer/dag i lavsæson. Du betaler 540.000 kr./år for 50% tomgang. En 3PL havde kostet 255.000 kr./år.

Valget er ikke "hvad føles rigtigt". Det er matematik. Her er regnestykket.

## Kort svar

> Under 80 ordrer/dag eller med stor sæsonvariation: vælg 3PL. Over 150 ordrer/dag med stabilt volumen: eget lager + WMS. Beregn break-even med dit faktiske volumen – det er matematik, ikke mavefornemmelse.

## Hvad er 3PL?

3PL (tredjepartslogistik) betyder, at du overlader opbevaring, pluk, pakning og forsendelse af dine ordrer til en ekstern leverandør. Du sender din beholdning til dem – de tager sig af resten.

Eget lager betyder du selv ejer eller lejer lagerarealerne og ansætter medarbejdere til at håndtere operationerne.

## Hvad koster det? Konkret regnestykke

**Scenario: 150 ordrer/dag, 2 linjer/ordre, 250 arbejdsdage**

| Post | 3PL | Eget lager + WMS |
|------|-----|------------------|
| Modtagelse (500 kolloer/md) | 500 × 6 kr. × 12 = 36.000 kr./år | Inkl. i lønomkostning |
| Opbevaring (40 paller) | 40 × 25 kr. × 52 uger = 52.000 kr./år | Husleje: 18.000 kr./md = 216.000 kr./år |
| Pluk (150 × 2 × 250) | 75.000 linjer × 8 kr. = 600.000 kr./år | 2 medarbejdere × 300.000 kr. = 600.000 kr./år |
| Pakning (150 × 250) | 37.500 pakker × 12 kr. = 450.000 kr./år | Inkl. i lønomkostning |
| Forsendelsestillæg | 37.500 × 8 kr. = 300.000 kr./år | 0 kr. (direkte til fragtleverandør) |
| WMS | 0 kr. (3PL's system) | 96.000 kr./år (SmartPack) |
| Udstyr (scannere, printere) | 0 kr. | 15.000 kr. (engangsinvestering, afskrives over 3 år = 5.000 kr./år) |
| **TOTAL/ÅR** | **1.438.000 kr.** | **917.000 kr.** |
| **Break-even** | – | **Eget lager sparer 521.000 kr./år** |

**Konklusion:** Ved 150 ordrer/dag med stabilt volumen: eget lager + WMS vinder. Ved under 80 ordrer/dag eller >3× sæsonvariation: 3PL vinder.

## Det opdager de fleste for sent

**1. 3PL-prisen stiger med volumen – men ikke lineært**  
Startpris: 65 kr./ordre. Ved 200 ordrer/dag: 75 kr./ordre ("volumenpåslag"). Ved 400 ordrer/dag: 95 kr./ordre ("kapacitetspåslag"). Du troede det blev billigere. Det blev dyrere.

**2. Eget lager kræver WMS – det er ikke optional**  
Du bygger eget lager. Kører Excel + Shopify. Ved 120 ordrer/dag: fejlrate 2,8% = 840.000 kr./år i fejlomkostninger. WMS koster 96.000 kr./år. Du sparer 744.000 kr. – men du opdager det først efter 18 måneders kaos.

**3. 3PL kan ikke håndtere dine specialkrav**  
Du sælger gaveindpakning. 3PL siger "vi kan det". Realitet: 40% af ordrer pakkes forkert. Kunder klager. Du mister 180.000 kr. i LTV. Skulle have beholdt eget lager.

**4. Eget lager binder kapital – også når du ikke vokser**  
Du investerer 450.000 kr. i lagerudstyr og 6 måneders husleje forudbetalt. Volumen stagnerer. Pengene er bundet. Du kan ikke pivotere. 3PL havde givet fleksibilitet.

## 3PL – fordele og ulemper

**Fordele:**
- Variabel omkostning – betaler pr. enhed/forsendelse
- Ingen kapitalinvestering i lagerareal og udstyr
- Nem skalering op og ned ved sæsonudsving
- Ekspertise i logistik – de gør dette til daglig
- Geografisk fleksibilitet – 3PL i UK løser din UK-levering

**Ulemper:**
- Mister direkte kontrol over kvalitet og oplevelse
- Dyrere per enhed ved højt, stabilt volumen
- Afhænger af 3PL's prioritering – du er én af mange kunder
- Sværere at håndtere specialkrav (unikke emballager, gaveindpakning)
- Data og synlighed er begrænset sammenlignet med eget lager

## Eget lager – fordele og ulemper

**Fordele:**
- Fuld kontrol over processer, kvalitet og medarbejdere
- Billigere per enhed ved stabilt, højt volumen
- Fleksibilitet til at tilpasse til særlige krav
- Direkte indblik i lageroperationen
- Stærkere brand-oplevelse (unikke esker, breve, tilpasning)

**Ulemper:**
- Fast omkostning – betaler også i lavsæson
- Kræver ansatte, udstyr og lokaler
- Svær at skalere hurtigt (lokaler og folk tager tid)
- Kræver WMS for at skalere effektivt over 50 ordrer/dag
- Ledelsestid og HR-ansvar

## Beslutningsmatrix

| Faktor | 3PL | Eget lager |
|--------|-----|-----------|
| Stabilt, højt volumen | ✗ | ✓ |
| Sæsonvariationer >3× | ✓ | ✗ |
| Kontrol og kvalitet | ✗ | ✓ |
| Hurtig international ekspansion | ✓ | ✗ |
| Startfase / lavt volumen | ✓ | ✗ |
| Specialprodukter/-pakning | ✗ | ✓ |

## Hvad koster 3PL?

Typiske 3PL-priser (DK-marked):

- **Modtagelse**: 3–8 kr. per kollo
- **Opbevaring**: 15–40 kr. per palle/uge
- **Pluk**: 5–15 kr. per ordrelinje
- **Pakning**: 8–20 kr. per pakke
- **Forsendelsestillæg**: 5–20 kr. over fragtpris

For en webshop med 100 ordrer/dag (2 linjer per ordre) kan 3PL-omkostningen let løbe op i 80–130 kr. per ordre.

## Hybridmodellen

Mange voksende webshops bruger en hybrid: eget primærlager i Danmark + 3PL i et udenlandsk marked (UK, DE). Det giver kontrol hjemme og skalering ude.

## Sådan gør du det rigtigt

1. **Beregn break-even baseret på faktisk volumen** – Brug regnestykket ovenfor med dine egne tal
2. **Kravspecificér SLA med 3PL** – Fejlrate under 0,5%, pakketid under 24 timer (skriv det ind i kontrakten)
3. **Kræv fuld API-integration** – Du skal se hvert ordrestatus i dit eget system, ikke kun i 3PL's portal
4. **Evaluer kvartalsvis** – Vækst ændrer kalkulationen. Tjek break-even hver 3. måned
5. **Overvej hybridmodel fra start** – Særligt ved internationale ambitioner

## Hvornår skal du handle?

**Handle nu hvis:**
- Du kæmper med kapacitet, rekruttering eller lagerplads – evaluer 3PL nu
- Du vil vokse internationalt inden for 12 måneder – undersøg 3PL i målmarkeder
- Din fejlrate stiger med volumen – eget lager + WMS eliminerer 83% af fejl

## SmartPack: Hvornår det giver mening (og hvornår det ikke gør)

**SmartPack giver mening hvis:**
- Du kører eget lager med 50–500 ordrer/dag
- Du vil have fuld kontrol over processer og kvalitet
- Du har stabile lagermedarbejdere (ikke 60%+ turnover)
- Du vil investere i skalering frem for at outsource

**SmartPack giver IKKE mening hvis:**
- Du har under 30 ordrer/dag og ingen vækstplan – Excel er OK
- Du har >3× sæsonvariation og ingen fast bemanding – brug 3PL
- Du vil ekspandere til 5 lande på 6 måneder – 3PL i hvert marked er hurtigere
- Du ikke har plads til lager eller kan leje lokaler – 3PL er eneste mulighed

**Konkret breakdown:**

| Volumen | 3PL cost/år | SmartPack + eget lager cost/år | Break-even |
|---------|-------------|-------------------------------|------------|
| 50 ordrer/dag | ~540.000 kr. | ~480.000 kr. | Eget lager vinder |
| 150 ordrer/dag | ~1.438.000 kr. | ~917.000 kr. | Eget lager vinder |
| 300 ordrer/dag | ~2.850.000 kr. | ~1.620.000 kr. | Eget lager vinder |

**Transfer-ordrer:** SmartPack understøtter multi-location. Hvis du kører hybrid (eget lager DK + 3PL i DE), kan SmartPack håndtere begge.


## Brug denne artikel
- [ ] Gennemgå listen og sæt kryds ved det der passer på din situation
- [ ] Beregn din fejlomkostning: antal fejl/md × 350 kr. = din månedlige tab
- [ ] Book 30 min med dit team til at gennemgå "Hvornår du skal handle"
- [ ] Næste skridt: kontakt SmartPack for en gratis gennemgang af dit setup

## FAQ

**Kan jeg skifte fra 3PL til eget lager?**
Ja – planlagte transitioner tager typisk 4–8 uger. Vær opmærksom på lagerbeholdning under flytningen.

**Hvad er en god 3PL-SLA?**
Fejlrate under 0,5%, forsendelse inden 24 timer, skadesrate under 0,1%.

**Skal jeg bruge eget WMS med en 3PL?**
Ja – dit system bør altid have synlighed ind i 3PL's operationer via API. Stol aldrig kun på 3PL's rapportering.

**Er 3PL en god løsning til startups?**
Ja – ingen kapitalinvestering, variabel omkostning og professionel håndtering fra dag ét.

**Hvad med returhåndtering ved 3PL?**
Det skal specificeres tydeligt i kontrakten. Returvurdering, genindsættelse på lager og destruktion af defekte varer skal alle have processer.

## Læs også

- [Hvornår fejler Excel som lagerstyring?](hvornår-excel-fejler.md)
- [Break-even på lagerinvestering](../05-Economics/break-even-lager.md)
- [Lagerstyring system i Danmark](lagerstyring-system-danmark.md)
- [Book en gratis SmartPack-demo](https://smartpack.dk/demo)

